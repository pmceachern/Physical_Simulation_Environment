...
def get_freqarray(f, Bopt, fmax, max_step, f_dense_low, f_dense_up, df_dense):
    ...
    if f < 0:
        ...
        f1_long = np.abs(f_dense_up)*np.power(k,(np.arange(1, Nlog_long+1)-1))-(np.abs(f_dense_up)-f_dense_up)
        ...
        f1_long = -(np.abs(f_dense_low)*np.power(k,np.arange(Nlog_long, 0, -1)-1))+(abs(f_dense_low)+f_dense_low)
...
def GN_integral(b2, Lspan, a_dB, gam, f_ch, rs, roll_off, power, Nch, model_param):
    ...
    f_dense_start = np.abs(np.sqrt(np.power(alpha_lin,2)/(4*np.power(np.pi, 4)*b2*b2)*(min_FWM_inv-1))/f2eval)
    f_ind_eval = 0
    ...
    for f in f_array:  # Loop over f
        ...
        f1_array = get_freqarray(f, Bopt, fmax, max_step, f_dense_low, f_dense_up, df) # Get non-uniformly spaced f1 array
        ...
        for f1 in f1_array:  # Loop over f1
            ...
            f2_array = get_freqarray(f, Bopt, fmax, max_step, f2_dense_low, f2_dense_up, df2) # Get non-uniformly spaced f2 array
            ...
            f_ind += 1
        GNLI[f_ind_eval] = 16/27*gam*gam*np.trapz(Gpart, f1_array)  # Compute outer integral. Nominal span loss already compensated
